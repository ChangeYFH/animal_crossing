<template>
  <div class="villagers-page">
    <mine-filter @search="refreshList" category="villagers" :conditions="conditions" :list="list"></mine-filter>
          ...
          <li class="item" v-for="(item,index) in list" :key="index" @click="showDetail(item.id)" >
            ...
            <div class="item-right">
              ...
              <div class="item-detail">
                <span class="race">种族:{{item.race}}</span>
              </div>
              <div class="item-detail">
                <span class="sex">{{item.sex}}</span>
                <span class="birthday">生日:{{item.birthday[1]}}月{{item.birthday[3]}}日</span>
                <span class="character">性格:{{item.character}}</span>
              </div>
            </div>
          </li>
        </ul>
    ...
    <villagers-detail ref="detail" :id="id" />
  </div>
</template>
...
<script>
  ...
  import VillagersDetail from "./villagersDetail";
  ...
  export default {
    ...
    methods:{
      ...
      getData(){
        ...
        Vue.http.get("http://62.234.172.175:3000/villagers",config).then(res => {
